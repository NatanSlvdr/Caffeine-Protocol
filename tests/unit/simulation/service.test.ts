    ...
    tickets: [
      {
        ...
        customer_id: 'C1',
        table_id: 'T01',
        source_phrase: 'coffee',
        source_intent: { drink: 'coffee' },
        item: 'coffee',
        with_sugar: false,
        sugar_count: 0,
        status: 'created',
...
describe('movement language and execution', () => {
  for (const command of ['MOVE UP 0', 'MOVE LEFT -1', 'MOVE DOWN 1.5', 'MOVE RIGHT 20', 'MOVE DIAGONAL 2'])
    it(`rejects ${command}`, () => expect(compileRobot(command, 'prep').compile_error).toContain('Unknown'));
  ...
  it('stops before furniture and continues with the next command', () => {
    ...
    const log = r.execution.events.filter((e) => e.actor === 'prep');
    const stopped = log.find((e) => e.command === 'MOVE LEFT 19' && e.start === e.end);
    expect(stopped?.completed).toBe(0);
    expect(stopped?.to).toEqual(STARTS.prep);
  ...
  it('does not enter another worker area or leave the room', () => {
    ...
    const log = r.execution.events.filter((e) => e.actor === 'prep');
    expect(log.find((e) => e.command === 'MOVE RIGHT 19' && e.start === e.end)?.to).toEqual([7, 5]);
    expect(log.find((e) => e.command === 'MOVE DOWN 19' && e.start === e.end)?.to).toEqual([7, 5]);
    ...
    for (const seed of r.execution ?? [])
      for (const e of seed.events.filter((e) => e.actor === 'prep' || e.actor === 'floor')) {
        if (!samePoint(e.from, e.to)) {
          expect(e.end - e.start).toBeCloseTo(1);
          expect(Math.abs(e.to[0] - e.from[0]) + Math.abs(e.to[1] - e.from[1])).toBe(1);
          expect(isWalkable(e.to, e.role)).toBe(true);
        }
      }
  });
    ...
    expect(
      log.some(
        (a) =>
          a.actor === 'prep' &&
          a.end > a.start &&
          log.some((b) => b.actor === 'floor' && b.start < a.end && b.end > a.start),
      ),
    ).toBe(true);
  });
