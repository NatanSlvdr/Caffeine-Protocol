...
import { CAMERA_POSITION,CAMERA_TARGET,CAMERA_ELEVATION,cameraZoom,ENTRANCE,FURNITURE,isWalkable,gridRoute,ROOM,BOUNDS,QUERY_TILES,MANUAL_INTAKE,STAFF_ENTRY,STATIONS,STARTS,TABLE_LAYOUT,tableFront,tableSeat,zoneAt,samePoint } from '../src/domain/layout';
import type { Point } from '../src/domain/layout';
import { SCENE_SIZE } from '../src/domain/layout';

describe('shared café geometry',()=>{
 it('matches the room plan and integer non-overlapping footprints',()=>{
  expect(ROOM).toEqual([16,12]);expect(TABLE_LAYOUT).toHaveLength(10);expect(TABLE_LAYOUT.every(t=>t.depth===1)).toBe(true);expect(new Set(TABLE_LAYOUT.map(t=>t.x))).toEqual(new Set([-5,0,5]));
  ...
  for(let z=BOUNDS.minZ;z<=3;z++)expect(isWalkable([3,z],'floor')).toBe(true);
 });
 ...
 it('brings customers from the bottom-left corner to the counter’s left side',()=>{
  ...
  for(let i=0;i<TABLE_LAYOUT.length;i++){
   ...
   expect(inbound).toEqual(outbound.slice().reverse());
 ...
 it('aligns the nearly overhead camera with the grid and frames scene bounds',()=>{
  ...
  for(const [w,h] of [[660,440],[490,440],[340,300]]){const zoom=cameraZoom(w,h);expect(zoom*(SCENE_SIZE[0]+1)).toBeLessThanOrEqual(w+1e-9);expect(zoom*(SCENE_SIZE[1]*Math.sin(CAMERA_ELEVATION)+3.4*Math.cos(CAMERA_ELEVATION)+1)).toBeLessThanOrEqual(h+1e-9);}
 });
});
