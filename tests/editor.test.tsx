...
describe('keyboard and mouse editor component behavior',()=>{
 ...
 it('inserts after the selected source line',async()=>{const user=userEvent.setup();render(<Harness initial={'LISTEN\nSUBMIT'}/>);await user.click(screen.getByLabelText('Block 1',{exact:true}));await user.click(screen.getByRole('button',{name:'Add block'}));expect(screen.getByLabelText('Current source').textContent).toBe('LISTEN\nTICKET\nSUBMIT');});
 it('replaces an instruction',async()=>{const user=userEvent.setup();render(<Harness initial={'LISTEN\nITEM coffee'}/>);await user.selectOptions(screen.getByLabelText('Block 2',{exact:true}),'ITEM tea');expect(screen.getByLabelText('Current source').textContent).toBe('LISTEN\nITEM tea');});
 it('moves and deletes single blocks',async()=>{const user=userEvent.setup();render(<Harness initial={'LISTEN\nTICKET\nSUBMIT'}/>);await user.click(screen.getByRole('button',{name:'Move block 2 down'}));expect(screen.getByLabelText('Current source').textContent).toBe('LISTEN\nSUBMIT\nTICKET');await user.click(screen.getByRole('button',{name:'Delete block 2'}));expect(screen.getByLabelText('Current source').textContent).toBe('LISTEN\nTICKET');});
 it('disables all mutations during playback',async()=>{const user=userEvent.setup();render(<Harness locked/>);for(const name of ['Add block','Move block 1 up','Move block 1 down','Delete block 1'])expect((screen.getByRole('button',{name}) as HTMLButtonElement).disabled).toBe(true);await user.click(screen.getByRole('button',{name:'Text'}));expect((screen.getByRole('textbox',{name:'Program source'}) as HTMLTextAreaElement).readOnly).toBe(true);});
