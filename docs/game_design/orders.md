...
- Level 4: coffee/tea membership; the first heard group is implicitly `item`.
...
Expected output remains a separate object with `item`, optional `with_sugar` or `sugar_count`, and optional `tickets` for grouped orders. From the sugar lesson onward, plain orders also validate that no sugar was added. Validation checks actual tickets in order, one physical sheet per requested drink, and completion of the return to the register. The speech bubble displays recognized groups rather than the expected answer.
