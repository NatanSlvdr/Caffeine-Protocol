import { GripVertical, Clock3, MoveUpRight, PenLine, ArrowRight, CircleHelp, CirclePlus, Coffee, CornerDownRight, GitBranch, Hand, ListRestart, MapPin, Package, Plus, ReceiptText, Repeat2, Settings2, SquareFunction, Ticket, Undo2, Waves } from 'lucide-react';
...
export function BlockIcon({ command }: { command: string }) {
  const icons = {
    ...
    STORE: GripVertical, CALL: SquareFunction, RETURN: Undo2, READ: ReceiptText, SUGAR: Plus, ADD: CirclePlus,
  ...
  if (family === 'DEPOSIT') return <svg className="block-icon deposit-icon" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    <path d="M12 3v11m-4-4 4 4 4-4M4 14v6h16v-6"/>
  </svg>;
