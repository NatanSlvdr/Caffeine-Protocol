...
import { cameraZoom, TABLE_LAYOUT, FURNITURE, STATIONS, STARTS, CAMERA_POSITION, CAMERA_TARGET, zoneAt, ROOM, BOUNDS } from '../domain/layout';
...
function Room({tables,evening}:{tables:number;evening:boolean}){return <group>
 ...
 {FURNITURE.filter(f=>f.kind!=='table'&&f.kind!=='chair').map(f=><group key={f.id} position={[f.x+(f.width-1)/2,0,f.z+(f.depth-1)/2]}>{f.kind==='plant'?<Plant at={[0,0,0]}/>:<><Box at={[0,.5,0]} size={[f.width,1,f.depth]} color="#527868"/><Box at={[0,1.05,0]} size={[f.width,.1,f.depth]} color="#e2c69c"/></>}</group>)}
 {Object.entries(STATIONS).map(([id,station])=><group key={id} position={[station.cell[0],0,station.cell[1]]}>
  ...
  :id==='ingredients'?<>{[-.24,.24].map(x=><group key={x}><Cylinder at={[x,1.4,0]} size={[.2,.2,.6]} color={x<0?'#80523b':'#75a45c'}/><Cylinder at={[x,1.72,0]} size={[.21,.21,.08]} color="#e2c585"/></group>)}</>
  :id==='grinder'?<><Box at={[0,1.3,0]} size={[.6,.4,.6]} color="#b39065"/><Cylinder at={[0,1.7,0]} size={[.27,.13,.4]} color="#4c372b"/></>
  :id==='water'?<><Box at={[0,1.12,0]} size={[.8,.06,.8]} color="#5b9fc1"/><Box at={[0,1.4,-.2]} size={[.09,.6,.09]} color="#bbd4d9"/><Box at={[.17,1.68,-.2]} size={[.4,.09,.09]} color="#bbd4d9"/></>
 ...
 {TABLE_LAYOUT.map((t,i)=><group key={t.id}><Table point={[t.x,t.z+(t.depth-1)/2]} depth={t.depth}/><Chair at={[t.x+1,0,t.z]} rotation={Math.PI/2}/><Box at={[t.x,.06,t.z+t.depth]} size={[.25,.035,.25]} color={i<tables?'#88a978':'#bba886'}/></group>)}
 ...
 <Box at={[-7,1.7,3]} size={[2,.12,.8]} color="#a78058"/>{[-7.5,-6.5].map(x=><Plant key={x} at={[x,1.8,3]} scale={.35}/>)}
...
function World({tables,evening,result,time,reduced,moving,level}:{tables:number;evening:boolean;result?:RunResult;time:number;reduced:boolean;moving:boolean;level:number}){
 ...
 {state?.customers.map((c,i)=><Character key={c.id} at={c.position} color={['#af7e67','#79929c','#b29c66'][i%3]}/>)}
