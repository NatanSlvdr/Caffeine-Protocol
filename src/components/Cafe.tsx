...
function CounterGate({ open }: { open: boolean }) {
  ...
  useFrame(() => {
    if (ref.current) {
      ...
      ref.current.rotation.y += (target - ref.current.rotation.y) * 0.16;
    }
  });
  return (
...
function Room({ evening, gateOpen, showLabels }: { evening: boolean; gateOpen: boolean; showLabels: boolean }) {
  return (
    <group>
...
function World({
  ...
  const actors: Partial<Record<ActorId, ActorSnapshot>> = state?.actors ?? {
    ...(level >= 3
  ...
  const gateOpen = !!state?.seed?.events.some(
    (e) =>
      e.actor === 'niko' &&
