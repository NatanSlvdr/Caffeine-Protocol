...
function World({evening,result,time,reduced,moving,level,showLabels,serviceView,focusRole}:{evening:boolean;result?:RunResult;time:number;reduced:boolean;moving:boolean;level:number;showLabels:boolean;serviceView:boolean;focusRole?:RobotRole}){
 ...
 {Object.entries(actors).map(([id,actor])=>actor&&<group key={id}><Character at={actor.position} robot={id==='query'||id==='prep'&&level>=15||id==='floor'&&level>=23} label={id==='prep'&&level<15?'Moka · Auto':id==='floor'&&level<23?'Pip · Auto':undefined} facing={actor.facing??(id==='query'?-Math.PI/2:0)} walking={moving&&actor.walking} reach={actor.reach} color={id==='floor'?'#d4ac6b':id==='prep'?'#7d9eae':'#80a889'} animate={moving} phase={time} reduced={reduced}/>{serviceView&&state&&!(id==='prep'&&level<15||id==='floor'&&level<23)&&(actor.heldPaper||actor.inventory.length>0||actor.action||Object.keys(actor.variables??{}).length)&&<Html transform={false} distanceFactor={1 / 70} position={[actor.position[0],2.8,actor.position[1]]} center zIndexRange={[10,0]} style={{pointerEvents:'none'}}><RobotHolding name={id==='niko'?'Niko':id==='query'?'Query':id==='prep'?(level>=15?'Brew':'Moka'):(level>=23?'Porter':'Pip')} inventory={actor.inventory} paper={actor.heldPaper} action={actor.action} variables={actor.variables} paused={!moving} reduced={reduced}/></Html>}{actor.inventory.map((item,i)=><Cup key={item.ticketId} at={[actor.position[0]-.2+i*.4,1.2,actor.position[1]+.3]} tea={item.item==='tea'}/>)}</group>)}
 {state&&<Html transform={false} distanceFactor={1 / 70} position={[STATIONS.orders.cell[0],2.7,STATIONS.orders.cell[1]]} center zIndexRange={[11,0]}><OrderQueueBubble tickets={state.waitingTickets}/></Html>}
 ...
 <StreetClip>{state?.customers.map((c,i)=>{
  ...
  {event&&c.showOrder&&<Html transform={false} distanceFactor={1 / 70} position={[c.position[0],2.2,c.position[1]]} center zIndexRange={[12,0]} style={{pointerEvents:'none'}}><CustomerSpeech customer={event.customer} clarified={clarified}/></Html>}
 </group>})}</StreetClip>
