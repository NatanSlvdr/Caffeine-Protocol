...
export function RobotHolding({ name, inventory, paper, action, variables, paused=false, reduced=false }: { name: string; inventory: Cargo[]; paper?: OrderTicket; action?:ActorSnapshot['action']; variables?:ActorSnapshot['variables']; paused?:boolean; reduced?:boolean }) {
  ...
  if (!paper && !inventory.length && !action && !memory.length) return null;
  ...
  return <div className="robot-holding" data-motion={reduced?'reduced':undefined} style={{animationPlayState:paused?'paused':'running'}} aria-label={`${name} is holding`}>
    {action&&<div className="robot-action" aria-label={`${name}: ${actionLabel}`}><span className="robot-action-icon" style={{transform:reduced?undefined:`translateY(${Math.sin(action.progress*Math.PI*8)*2}px) rotate(${Math.sin(action.progress*Math.PI*6)*10}deg)`}}><BlockIcon command={action.command}/></span><span>{actionLabel}</span><progress aria-label="Action progress" value={action.progress} max={1}/></div>}
    <ul>
      {paper && <li key={`${paper.ticket_id}:${paper.item}:${paper.quantity}:${paper.sugar_count}`} title={paperLabel} aria-label={paperLabel}><HoldingIcon item={paper.item} stage="paper"/>{(paper.quantity??1)>1&&<span className="order-quantity">×{paper.quantity}</span>}{paper.sugar_count!==null&&<span className="holding-sugar"><Sparkles size={10}/>{paper.sugar_count}</span>}</li>}
      {inventory.map(cargo => {
