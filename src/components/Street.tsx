...
/** Traffic loops beyond the diorama, scaling out at the boundary. */
function Car({ lane, direction, offset, color, paused, reduced }: StreetMotion & { lane: number; direction: number; offset: number; color: string }) {
  ...
  useFrame((_, delta) => {
    ...
    const z = ((elapsed.current * 1.4 + offset) % loop - loop / 2) * direction + STREET_BOUNDS.centerZ;
    ref.current.position.z = z;
    ref.current.scale.setScalar(Math.max(0, Math.min(1, (STREET_BOUNDS.length / 2 - Math.abs(z - STREET_BOUNDS.centerZ)) / 1.2)));
  });
...
function Pedestrian({ offset, direction, color, paused, reduced }: StreetMotion & { offset: number; direction: number; color: string }) {
  ...
  useFrame((_, delta) => {
    ...
    ref.current.scale.setScalar(Math.max(0, Math.min(1, STREET_BOUNDS.length / 2 - Math.abs(z - STREET_BOUNDS.centerZ))));
    legs.current?.children.forEach((leg, i) => { leg.rotation.x = reduced ? 0 : Math.sin(elapsed.current * 5 + offset + i * Math.PI) * .35; });
  });
  return <group ref={ref} position={[direction > 0 ? -10.4 : -10.85, 0, 0]} rotation-y={direction > 0 ? 0 : Math.PI}>
...
function StreetLamp({ z, evening }: { z: number; evening: boolean }) {
  return <group position={[-11.12, 0, z]}>
...
export function Street({ paused, reduced, evening }: StreetMotion & { evening: boolean }) {
  ...
  return <group>
    ...
    <Box at={[-10.08, -.37, STREET_BOUNDS.centerZ]} size={[2.84, .78, STREET_BOUNDS.length]} color="#e9d7ba"/>
    ...
    <Pedestrian direction={1} offset={13} color={CAFE_COLORS.sage} {...motion}/>
  </group>;
}
