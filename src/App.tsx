...
import { Store, ArrowLeft, ArrowRight, Coffee, Download, FolderHeart, HelpCircle, Home, Leaf, Maximize, Pause, Play, RotateCcw, Settings2, SlidersHorizontal, Sparkles, Square, Star, Terminal, Upload, Volume2 } from 'lucide-react';
import { Cafe } from './components/Cafe';
import { CampaignTrail } from './components/CampaignTrail';
...
export default function App(){
  ...
  <div className="app-body">{screen!=='home'&&screen!=='workspace'&&screen!=='campaign'&&<nav className="rail" aria-label="Main navigation"><button aria-label="Main menu" onClick={()=>go('/')}><Home size={21}/></button><button aria-label="Campaign" onClick={()=>go('/campaign')}><FolderHeart size={21}/></button><div className="rail-divider"/><span className="rail-label">CAFÉ</span><button aria-label="Selected shift" onClick={()=>launch(save.selected)}><Terminal size={21}/></button><span className="rail-spacer"/><button aria-label="Game guide" onClick={()=>setModal('guide')}><HelpCircle size={21}/></button><button aria-label="Audio and display settings" onClick={()=>go('/settings')}><SlidersHorizontal size={21}/></button><Leaf className="rail-leaf" size={19}/></nav>}
  {screen==='home'&&<LandingPage save={save} onContinue={()=>launch(save.selected)} onExplore={()=>go('/campaign')}/>}
  {screen==='campaign'&&<CampaignTrail save={save} onSelect={selected=>setSave(s=>({...s,selected}))} onLaunch={launch} onEnding={()=>go('/ending')}/>}
