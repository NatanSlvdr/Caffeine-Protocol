...
export const CAMERA_POSITION: [number,number,number]=[CAMERA_TARGET[0],CAMERA_TARGET[1]+40,CAMERA_TARGET[2]+40/Math.tan(CAMERA_ELEVATION)];
export type Furniture = {id:string;kind:'counter'|'table'|'chair'|'plant';x:number;z:number;width:number;depth:number};
/** Ten service tables in staggered groups, with a clear cross-aisle before the counter. */
export const TABLE_LAYOUT=[[-5,-5],[0,-5],[5,-5],[-5,-3],[5,-3],[-5,-1],[0,-1],[5,-1],[0,1],[5,1]].map(([x,z],i)=>({id:`T${String(i+1).padStart(2,'0')}`,x,z,width:1,depth:1}));
...
export const STATIONS = {
 ...
 dock:{cell:[3,2],floor:[3,3],label:'CHARGE'},
} as const;
...
export function cameraZoom(width:number,height:number){return Math.max(1,Math.min(width/(SCENE_SIZE[0]+1),height/(SCENE_SIZE[1]*Math.sin(CAMERA_ELEVATION)+3.4*Math.cos(CAMERA_ELEVATION)+1)));}
