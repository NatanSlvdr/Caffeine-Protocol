...
import { INSTRUCTION_LIMIT, MAX_TRANSITIONS, SIM_DURATION_SECONDS } from './constants';
...
export function* streamService(level:LevelDefinition,events:ReplayEvent[],programs:RobotPrograms,start=0,live?:LiveService):Generator<number,ServiceResult>{
 ...
 const workers:Worker[]=(['prep','floor'] as const).map(role=>({role,actor:role,program:compileRobot(number>=ROBOT_UNLOCK_LEVELS[role]?programs[role]:role==='prep'?preparationSource(32):floorSource(32),role,number>=ROBOT_UNLOCK_LEVELS[role]?number:32),pc:0,stack:[],position:STARTS[role],inventory:[],count:0,maxLoad:0,done:false}));
 ...
 if(!live)for(const [index,event] of events.entries()){
  ...
  for(const [ticketIndex,ticket] of (event.passed?event.tickets:[]).entries()){
   ...
   for(const unit of ticketUnits(ticket))jobs.push({ticketId:unit.ticket_id,table:event.table,item:ticket.item as 'coffee'|'tea',sugar:sugarOf(ticket),event,created:level.programming_enabled?handoff:Infinity,status:'ticket',dirtyAt:Infinity});
  }
 }
 ...
 const capacity=(w:Worker)=>w.role==='prep'?config.prepCapacity:config.floorCapacity;
  const condition=(w:Worker,c:string)=>{const job=currentJob(w);const comparison=parseWorkerComparison(`IF ${c}`);if(comparison){const speech={drink:job?.item,with_sugar:(job?.sugar??0)>0,sugar_count:job?.sugar};return evaluateWorkerComparison(comparison,speech,speech);}return c==='coffee'?job?.item==='coffee':c==='tea'?job?.item==='tea':c==='sugar'?(job?.sugar??0)>0:c.startsWith('TABLE ')?job?.table===Number(c.slice(6)):false;};
 ...
 const step=(w:Worker):boolean=>{
  ...
  if(++w.count>INSTRUCTION_LIMIT){fail(w,'Instruction limit reached (10,000 per robot).');return false;}
  if(isMoveCommand(c)){
   const move=parseMoveCommand(c);
   if(!move){fail(w,`Unknown movement direction: ${c.split(' ')[1]}`,line);return false;}
   const direction:Point=directionVectors[move.direction];
   w.move={started:now,from:w.position,direction,remaining:move.count,requested:move.count,completed:0,line,command:c};return true;
  }
  ...
  if(c==='WAIT TICKET'){
   ...
   if(take){
    ...
    if(take.direction!=='UP'){fail(w,'Take ingredients upward from the storage counter.');return false;}
 ...
 while(!failure&&now<=SIM_DURATION_SECONDS&&transitions++<MAX_TRANSITIONS){
  ...
  if(!live&&!level.programming_enabled&&!manualIntake&&manualIndex<events.length&&events[manualIndex].customer.arrival<=now){
  ...
  if((!live||live.done())&&finished()&&workers.every(w=>!w.pending)&&workers.every(w=>!w.move)&&now>=intakeFree&&(config.objective!=='serve'||events.every(event=>!event.tickets.length||now>=event.timing.left+STREET_EXIT_SECONDS))){
   const floor=workers[1],loadWorker=number<ROBOT_UNLOCK_LEVELS.floor?workers[0]:floor;
  ...
  const future=[...events.flatMap(event=>[event.timing.created,event.timing.seated,event.timing.left,event.timing.left+STREET_EXIT_SECONDS]).filter(t=>t>now),...(live?[live.next()]:[]),...(queryFailure&&queryFailure.timing.created>now?[queryFailure.timing.created]:[]),...(manualIntake?[manualIntake.end]:[]),...(!live&&!level.programming_enabled&&manualIndex<events.length?[events[manualIndex].customer.arrival]:[]),...workers.flatMap(w=>w.pending?[w.pending.end]:[]),...jobs.filter(j=>j.status==='ticket'&&j.created>now).map(j=>j.created),...jobs.filter(j=>j.status==='served'&&j.dirtyAt>now).map(j=>j.dirtyAt)].filter(t=>Number.isFinite(t)&&t>now);
 ...
 if(!failure&&(now>SIM_DURATION_SECONDS||transitions>=MAX_TRANSITIONS))fail(workers[0],'Simulation limit reached (3,600 seconds).');
 ...
 return {execution:{seed_id:seed,start,duration:failure?Math.max(.1,Math.min(failure.time,SIM_DURATION_SECONDS)):Math.max(1,Math.min(now,SIM_DURATION_SECONDS),...log.map(e=>e.end)),events:log},failure,instructions:workers.filter(w=>number>=ROBOT_UNLOCK_LEVELS[w.role]).reduce((n,w)=>n+w.count,0)};
}
