...
export interface CustomerExecution { variables?:Record<string,number|undefined>; heldPaper?:OrderTicket; payment?:OrderPayment;  tickets: OrderTicket[]; asked_help: boolean; error: string; error_line?: number; executed_instructions: number; trace: TraceStep[]; state: RuntimeState }
...
export interface ActorSnapshot { action?:{command:string;progress:number;start:number}; variables?:Record<string,number|undefined>; facing?:number; walking?:boolean; reach?:number; heldPaper?:OrderTicket; position:readonly [number,number]; inventory:Cargo[]; role:RobotRole }
export interface ExecutionEvent { variables?:Record<string,number|undefined>; heldPaper?:OrderTicket; ticketId?:string;  seed_id:string; actor:ActorId; role:RobotRole; start:number; end:number; line:number; command:string; from:readonly [number,number]; to:readonly [number,number]; inventory:Cargo[]; requested?:number; completed?:number; error?:string; customerId?:string }
