...
export function compileRobot(source:string,role:RobotRole,level=32):Program{
 ...
 for(const [line,raw] of source.split('\n').entries()){
  ...
  if(!allowed.includes(c)&&!movePattern.test(c)&&!(role==='prep'&&directionalDepositPattern.test(c))&&!(role==='floor'&&directionalPickupPattern.test(c))&&!legacyAction&&!comparisonUnlocked(c, level))return fail(`Unknown or locked ${role} instruction: ${c}`,line);
