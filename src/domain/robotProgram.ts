...
export function robotCommands(role:RobotRole,level:number):string[]{
 ...
 const common=['MOVE UP 1','MOVE DOWN 1','MOVE LEFT 1','MOVE RIGHT 1','REPEAT','IF coffee','IF tea','IF sugar','ELSE','END'];
 if(role==='prep')return [...common,'WAIT TICKET','TAKE BEANS','TAKE LEAVES','GRIND','FILL WATER','BREW','STEEP','ADD SUGAR','DEPOSIT',...(level>=20?['FUNCTION recipe','CALL recipe','RETURN']:[])];
 return [...common,'FUNCTION deliver','CALL deliver','FUNCTION clear','CALL clear','RETURN','WAIT DRINK','PICKUP','SERVE',...Array.from({length:10},(_,i)=>`IF TABLE ${i+1}`),...(level>=23?['WAIT DIRTY','COLLECT','RETURN CUPS']:[]),...(level>=27?['CHARGE','IF BATTERY < 40']:[])];
}
export function compileRobot(source:string,role:RobotRole,level=32):Program{
 ...
 for(const [line,raw] of source.split('\n').entries()){
  ...
  if(!allowed.includes(c)&&!/^MOVE (UP|DOWN|LEFT|RIGHT) ([1-9]|1[0-9])$/.test(c))return fail(`Unknown or locked ${role} instruction: ${c}`,line);
