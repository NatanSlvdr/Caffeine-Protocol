...
import { QUERY_PROGRAM_LEVEL_CAP, ROBOT_MAX_BLOCKS } from './constants';
...
export function robotCommands(role:RobotRole,level:number):string[]{
 ...
 const common=[...DIRECTIONS.map(direction=>`MOVE ${direction} 1`),'REPEAT',...(level>=4?['IF coffee IN CUSTOMER SPEECH']:[]),'IF coffee','IF tea','IF sugar','ELSE','END'];
 ...
 return [...common,'FUNCTION deliver','CALL deliver','FUNCTION clear','CALL clear','RETURN','WAIT DRINK','TAKE DOWN',...DIRECTIONS.filter(direction=>direction!=='DOWN').map(direction=>`TAKE ${direction}`),'SERVE',...Array.from({length:TABLE_LAYOUT.length},(_,i)=>`IF TABLE ${i+1}`),...(level>=ROBOT_UNLOCK_LEVELS.floor?['WAIT DIRTY','COLLECT','RETURN CUPS']:[])];
}
export function compileRobot(source:string,role:RobotRole,level=32):Program{
 ...
 for(const [line,raw] of source.split('\n').entries()){
  ...
  if(!allowed.includes(c)&&!MOVE_RE.test(c)&&!(role==='prep'&&DEPOSIT_RE.test(c))&&!(role==='floor'&&TAKE_RE.test(c))&&!legacyAction&&!comparisonUnlocked(c, level))return fail(`Unknown or locked ${role} instruction: ${c}`,line);
 ...
 p.block_count=p.instructions.length;if(p.block_count>ROBOT_MAX_BLOCKS)return fail('Moving robots have room for 512 blocks.',p.source_lines[512]);
