...
export function floorSource(level:number,batch=1,tableCount=10){
 ...
 return [...Array.from({length:batch},()=>['WAIT DRINK','PICKUP']).flat(),...Array.from({length:batch},()=>['CALL deliver',...charge]).flat(),...(level>=23?Array.from({length:batch},()=>['CALL clear',...charge]).flat():[]),'REPEAT','FUNCTION deliver',...serve,'RETURN','END',...(level>=23?['FUNCTION clear',...clear,'RETURN','END']:[])].join('\n');
}
