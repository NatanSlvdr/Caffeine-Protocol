...
export function runLevel(level: LevelDefinition, program: Program, robotPrograms?: RobotPrograms): RunResult {
  ...
  result.execution = [];
  let offset = 0;
  const orderPassed = result.passed;
  ...
  result.average_satisfaction = result.events.length
    ...
    : 100;
  if (result.passed && level.programming_enabled) {
    result.stars = 1;
    if ((result.block_count ?? 0) <= level.block_target) {
      result.stars = 2;
      if (result.executed_instructions <= level.instruction_target) result.stars = 3;
    }
  }
  return result;
}
