...
export function runLevel(level:LevelDefinition,program:Program,robotPrograms?:RobotPrograms):RunResult {
  ...
  outer:for(const seed of level.seeds){
    ...
    for(const customer of seed.customers){
      ...
      if(reason&&!actual.error){const prefix=reason.includes('checkout')||reason.includes('payment')?'DEPOSIT':reason.includes('item')?'ITEM':reason.includes('sugar')?'SUGAR':'DEPOSIT';line=actual.trace.findLast(s=>s.command.startsWith(prefix))?.line??line;}
