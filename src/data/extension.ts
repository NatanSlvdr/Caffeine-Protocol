...
export const extensionLevels:LevelDefinition[]=titles.map((title,i)=>{
 ...
 const seeds=Array.from({length:3},(_,seed)=>{
  ...
  const customers:Customer[]=Array.from({length:count},(_,n)=>{
   ...
   if(level===32&&n%4===0)return {customer_id:`C${n+1}`,arrival:n*4,phrase:'Our usual, please',intent:{confidence:'ambiguous'},clarification:`${drink}, ${sugar_count} sugars`,clarification_intent:intent,expected:{item:drink,sugar_count,ask_help:true}};
   if(level===32&&n%4===1)return {customer_id:`C${n+1}`,arrival:n*4,phrase:'Coffee and tea, please',intent:{orders:[{drink:'coffee',sugar_count:0},{drink:'tea',sugar_count:2}]},expected:{tickets:[{item:'coffee',sugar_count:0},{item:'tea',sugar_count:2}]}};
   return {customer_id:`C${n+1}`,arrival:n*(level>=31?4:10),phrase:`${drink}, ${sugar_count} sugars`,intent,expected:{item:drink,sugar_count}};
  });
  // Batch lessons have an even number of tickets, including grouped final orders.
  if(customers.reduce((n,c)=>n+(c.expected.tickets?.length??1),0)%2)customers.push({customer_id:`C${count+1}`,arrival:count*4,phrase:'Coffee, no sugar',intent:{drink:'coffee',sugar_count:0},expected:{item:'coffee',sugar_count:0}});
