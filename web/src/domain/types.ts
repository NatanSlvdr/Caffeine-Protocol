...
export interface LevelDefinition { id: string; title: string; programming_enabled: boolean; block_target: number; instruction_target: number; reference_block_count: number; seeds: ValidationSeed[]; summary: string; active_tables: number }
export interface ProgramBlock { id: string; command: string; sourceLine: number }
...
export interface RuntimeState { pc: number; stopped: boolean }
export interface CustomerExecution { tickets: OrderTicket[]; asked_help: boolean; error: string; error_line?: number; executed_instructions: number; trace: TraceStep[]; state: RuntimeState }
...
export interface ReplayEvent { seed_id: string; customer: Customer; tickets: OrderTicket[]; asked_help: boolean; passed: boolean; reason?: string; trace: TraceStep[]; failure_line?: number; timing: Timing; table: number; satisfaction: number }
export interface RunFailure { seed_id: string; error_line: number; customer_id: string; event_time: number; phrase: string; intent: SpeechIntent; expected: Customer['expected']; actual: OrderTicket[]; reason: string }
export interface RunResult { passed: boolean; observation: boolean; events: ReplayEvent[]; block_count?: number; level_id: string; level_title: string; passed_seeds: number; required_seeds: number; tickets: OrderTicket[]; executed_instructions: number; average_satisfaction: number; stars: number; first_failure: RunFailure | null }
